...
import { forexSchedule, nextResetScan } from '../../vps-backend/automation/optionsStrategies.js';
...
export default function ResearchPulse({ state }) {
  const now=useNow(), schedule=forexSchedule(now/1000), resetScan=nextResetScan(now/1000);
  const forexPositions=(state?.positions||[]).filter(p=>p.family==='forex').length;
  const resetPositions=(state?.positions||[]).filter(p=>p.family==='reset').length;
  return <div className="research-pulse" aria-label="Agenda ao vivo do laboratório">
    ...
    <article className="market-clock is-open"><Clock3/><div><span>Daily Reset · mercado 24/7</span><strong>{remaining(resetScan,now)}</strong><small>{resetPositions?`${resetPositions} par(es) em apuração`:'Viés Bull/Bear × controle oposto'}</small></div></article>
  </div>;
}
