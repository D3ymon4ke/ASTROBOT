...
export default function ResearchPanel({ research, available, pending, busy, symbols, symbol, onSymbol, onConfigure, onReplay, result }) {
  ...
  return <section className="aut-card">
    ...
    <p>Ativos da captura: {(research?.enabled ? research.symbols : symbols).join(', ')}. Rompimento v1: canal de seis velas, fechamento confirmado, durações de 1 e 3 minutos e proposta de USD 0,35, somente em observação.</p>
    ...
    {result?.directionBenchmark && <section className="aut-card"><h2>Direção do sinal × referência aleatória</h2><p>{result.directionBenchmark.warning}</p><div className="aut-table"><table><thead><tr><th>Estratégia / duração</th><th>Pares válidos</th><th>Excluídos</th><th>Resultado do sinal</th><th>Resultado esperado 50/50</th><th>Diferença</th></tr></thead><tbody>{result.directionBenchmark.groups.map(g => <tr key={g.name}><td>{g.name}</td><td>{g.pairs}</td><td>{g.excluded}</td><td>{g.signalNet.toFixed(2)} USD</td><td>{g.randomExpectedNet.toFixed(2)} USD</td><td>{(g.signalNet-g.randomExpectedNet).toFixed(2)} USD</td></tr>)}</tbody></table></div>{!result.directionBenchmark.groups.length && <p>Aguardando captura de propostas pareadas da nova versão.</p>}</section>}
