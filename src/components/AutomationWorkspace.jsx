...
export default function AutomationWorkspace({ children, continuous, timelineEnabled, timelineTrades = [], accountMode = 'demo', initialTab = 'timeline', onTabChange }) {
  ...
  useEffect(() => { setDirty(false); setLabResult(null); setNotice(''); }, [accountMode]);
  useEffect(() => {
    const previousLab = derivAPI.onLaboratoryResult, previousMessage = derivAPI.onAutomationMessage;
    derivAPI.onLaboratoryResult = result => { setLabResult(result); setSource('backtest'); setLabBusy(false); setLabError(''); };
