...
export default function App() {
  ...
  const [cycles, setCycles] = useState(() => {
    ...
    // Default seeded cycles - 6 calibrated strategic windows
    return [
    ...
    <div className="premium-app" style={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '100%', background: '#06080E', overflow: 'hidden' }}>
      ...
      {(() => {
        if (activePage === 'dashboard') {
          return <OperationsDashboard isDemo={isDemo} balance={balance} sessionProfit={initialBalance > 0 ? netProfit : 0}
            running={isRunning} connected={connected} brokerConnected={derivAPI.derivConnected && derivAPI.derivAuthorized}
            settings={settings} continuous={continuousState} research={researchState} timelineEnabled={schedulerState}
            timelineTrades={dbTrades} cycles={cycles} activeTrade={activeTradeCountdown || stateRef.current.lastContractDetails || continuousState?.legacyOrder}
            assets={ASSETS_LIST} blacklist={activeBlacklist} onAddBlacklist={handleAddBlacklist} onRemoveBlacklist={handleRemoveBlacklist}
            onStart={startBot} onStop={stopBot} onResults={() => setShowSessionResultsModal(true)} onNavigate={setActivePage}
            onAutomation={tab => { setAutomationTab(tab); setActivePage('automation'); }} />;
        }
          ...
          return (
            <main style={{ padding: '1.25rem', flex: 1, overflowY: 'auto' }}>
              ...
              <Scheduler
                ...
                onSendDecisionTreeTelegram={(cycleId) => {
                  derivAPI.sendDecisionTreeTelegram(cycleId);
                }}
