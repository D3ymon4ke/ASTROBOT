...
test('analyzeQuantumTrend detects bullish EMA ribbon breakout', () => {
  const candles = [];
  for (let i = 0; i < 40; i++) {
    const base = 1000 + i * 5 + (i % 2 === 0 ? 1 : -1);
    candles.push({
      open: base,
      high: base + 6,
      low: base - 2,
  ...
  assert.ok(analysis.indicators.ema9 > analysis.indicators.ema21);
});
...
test('DigitTrader simulates M1 CALL/PUT trades with 95% payout, Fakegale gate, and session lock', async () => {
  const session = {
    activeMode: 'demo',
        ...
        if (req.proposal) {
          return {
            proposal: {
              ...
              payout: req.amount * 1.95, // 95% net payout
              spot: 1050.00,
  ...
  trader.configure({
    ...
    maxGale: 1,
    minScore: 75,
    sessionTarget: 0.50, // Small target to lock after 1 win
    cooldownMinutes: 30,
    enableFakegaleLoss: true,
    symbols: ['R_100']
  });

  // Step 1: First scan registers virtual observation (Fakegale filter)
  trader.lastPoll = 0;
  await trader.tick();
  assert.equal(trader.state.pending.length, 0); // Fakegale gated first trigger
  ...
  // Session profit target hit -> Cooldown active!
